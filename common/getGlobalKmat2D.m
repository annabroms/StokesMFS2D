function K = getGlobalKmat2D(r,q)
%GETGLOBALKMAT2D(r,q) returns the matrix K that maps a vector of RBM to velocities at the boundary 
...
for i = 1:P %loop over particles
    ...
    K((i-1)*N+1:i*N,3*(i-1)+1:3*i) = K_block(1:end/2,:);
    K(P*N+(i-1)*N+1:P*N+i*N,3*(i-1)+1:3*i) = K_block(end/2+1:end,:);
end
