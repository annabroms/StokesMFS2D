% Compare resistance big-sparse solves with sparse_map_coarse off and on.
...
runs = repmat(struct('FT',[],'sol',struct()),2,1);
for sparse_map_coarse = [false true]
    ...
    runs(1 + sparse_map_coarse) = struct('FT',FT,'sol',sol);
...
if report_output_comparison
    fprintf(['  relerr(FT on/off)=%.3e  resvec diff=%.3e  ', ...
        'it=%d/%d\n'], ...
        relerr(runs(2).FT,runs(1).FT), ...
