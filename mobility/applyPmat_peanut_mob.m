function res = applyPmat_peanut_mob(vel,rvec_in,rvec_out,L,Sinv,q,Zi,Yi,rbase_in_c,...
    rbase_in_f,refine,rimage,nimage,opt,UU,YY,pairs,UB_all,YB_all,UC_all, YC_all,Cmap,Lc_pair,Lf_pair,Vk)
%APPLYPMAT  Apply long-range preconditioning projection matrix to a velocity field.
%
%   res = APPLYPMAT_PEANUT_MOB(vel, rvec_in, rvec_out, Rinv, Zi, Yi, opt)
%
...
mu = getCoarseMu(vel,Sinv,Zi,Yi,opt.db,opt.P,opt.N_c,opt.a_c,Vk);
...
proj_vel = matvec_mob_pairprecond_peanut(mu,rbase_in_c,rbase_in_f,rvec_in,...
    refine,rimage,nimage,opt,rvec_out,rvec_out,q,UU,YY,L,pairs,UB_all,YB_all,UC_all, YC_all,Cmap,Lc_pair,Lf_pair);
%UU{1} = UU{1}'; %stupid
