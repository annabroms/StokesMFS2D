...
PM = length(rvec_out);
N_c = opt.N_c; 
PM_check = length(rcheck);
N_large = PM_check/P;


%% Transform coarse \mu -> coarse \lambda (interior source strengths)
...
if on_coarse_grid
    ...
    if opt.cmap % used compressed mapping: correction_to_velocity <- course boundary data
        ...
        for pair_it = 1:size(pairs,1)
            ...
            % Determine rigid body motion for the pair
            if use_pair_cache
                % rotate to canonical reference frame and back again
        ...
        % Use the actual fine sources to determine correction to velocity
