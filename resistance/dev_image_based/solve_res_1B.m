function [FT,lambda, it, gmres_tol, maxres] = solve_res_1B(q,U,W,rad,image,lr,visualise,gmres_tol,debug,gmres_verbose)
%SOLVE_RES_1B Solves a 2D Stokes resistance problem with circular particles
% using 1-body preconditioned MFS.
%
% Syntax:
%   [FT, lambda, it, gmres_tol, maxres] = solve_res_1B(q, U, W, rad, image, visualise)
%
% Inputs:
...
if lr
  ...
  %  maxit = 1; %debug
   % [x_gmres,it,resvec,real_res] = helsing_gmres(@(x) Pmat*matvec_res_Stokes(x,rin,rout,rimage,nimage,q,Uii,Yii,pair_points,s),Pmat*fout,2*size(rout,1),maxit,gmres_tol,1,rout);
        ...
        [x_gmres,it,resvec,real_res] = helsing_gmres(@(x) lr_matvec_2D_Stokes(x,rin,rout,rimage,nimage,q,Uii,Yii,pair_points,s,Sinv,Nx,Ny,Mx,Z,Y,opt),Pf,2*size(rout,1),maxit,gmres_tol,opt.gmres_verbose,rout);
