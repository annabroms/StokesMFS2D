...
%   false (default): keep the legacy pair_tau + source_scatter_rows update.
%   true:            build a direct global M_source_corr in addition to the
%                    pair-local maps that are still needed for u_corr and
%                    M_ft_corr.
...
sparse_cols = double(sparse_map_coarse)*n_source_cols + ...
    n_source_cols + n_source_cols + n_source_cols + ...
    pair_total_rows;
