function tau_coarse = getCoarseSource(f,Rinv,Nx,Ny,Mx,Z,Y,db,P,Nc,a)
...
for k = 1:P
   ...
   tau_coarse((k-1)*Nc+1:k*Nc) = res(1:end/2);
   tau_coarse((k-1)*Nc+1+P*Nc:k*Nc+P*Nc) = res(end/2+1:end);
