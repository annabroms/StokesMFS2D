...
for i = 1:P
    ...
    % Coarse collocation data on particle i.
    tau_particle_x = tau((i-1)*N_large+1:N_large*i);
    tau_particle_y = tau(PM+(i-1)*N_large+1:PM+N_large*i);

    % One-body pseudoinverse map to coarse source strengths at proxy boundary.
    ...
    % Identify neighbours from pair list.
    if ~isempty(pairs)
        [I,J] = find(pairs==i);
        neigh = [I J];
    else
    ...
    if ~isempty(neigh)
        for k = 1:size(neigh,1)
            ...
            % Coarse collocation data on neighbour particle.
            ...
            % Build chi_1,2 and chi_2,1 jointly.
