function [Rinv,Nx,Ny,Mx,Z,Y,db] = get_long_range_precond(q,rin,rout,opt)
...
%% Is it a good choice to take eigenvectors of G for a singel body to be the coarse basis functions in Z? If so, what chould Y be?
...
%Y = MM; 
    ...
    Y = [Mx zeros(size(Mx)); zeros(size(Mx)) Mx];
    Z = [Nx zeros(size(Mx)); zeros(size(Mx)) Ny];
end
...
for k = 1:P
    ...
    for i = 1:db
        if svd_type
            src_struct(i,1,:) = Z(1:end/2,i);
            src_struct(i,2,:) = Z(end/2+1:end,i);
        else
...
for k = 1:P
    R((k-1)*db+1:k*db,:) = Y'*[Vmat((k-1)*Nc*a+1:k*Nc*a,:); Vmat(P*Nc*a+(k-1)*Nc*a+1:P*Nc*a+k*Nc*a,:)]; 
end
