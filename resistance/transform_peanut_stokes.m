function [tau_stokes_x, tau_self_x, tau_beta_x,tau_cf_x,tau_stokes_y,tau_self_y,tau_beta_y,tau_cf_y,u_corr] = transform_peanut_stokes(tau,rbase_in_c,rbase_in_f,rbase_out_f,refine,rimage_vec,opt,rvec_out,rcheck_out,q,U,Y,pairs,Upf,Ypf,DC_all, YC_all,Cmap,exterior,debug)
...
% Phase 1: determine course sources from boundary data
for i = 1:P
...
%                 tau_cf_y((p2-1)*N_c+1:N_c*p2) = tau_cf_y((p2-1)*N_c+1:N_c*p2)+tau_peanut_stokes(3*N_c+1:4*N_c);
            ...
            if two_parts 
                ...
                N_pair = singleLayer(rin_pair_f,rout_pair,mu);           
                ...
                [u1,v1] = stokesletDirect(real(rin_pair_f),imag(rin_pair_f),...
                    ...
                    tau_mapped_tot(1:end/2),tau_mapped_tot(end/2+1:end),N_im);
                u_pair = [u1; v1];
